...
graph LR
    subgraph "User Interface (View)"
        A[Ввод команды] --> B(Вызов ExecuteCommand)
    end

    subgraph "ViewModel (Логика)"
        B --> C{Анализ команды};
        C -->|'спроси'| D[AIService]
        C -->|'dir', 'читать', ...| E[Файловые операции]
        C -->|'открыть сайт'| F[Запуск процесса]
        C -->|'посчитать'| G[Вычисление выражения]
    end

    subgraph "Services & APIs"
        D --> H[CacheService]
        H -- "Промах кэша" --> I[OpenRouter API]
    end

    subgraph "Обработка результата"
        I -- "Ответ от API" --> J{Результат}
        H -- "Данные из кэша" --> J
        E --> J
        F --> J
        G --> J
    ...
    J --> K[Обновление Output и History в ViewModel] --> L[Отображение в UI]
```
